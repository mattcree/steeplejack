...
TEST_CASE("Tuning: Acceptance 1: all five shipped tuning files load without error")
{
    ...
    CHECK(t.Sources().size() == 9);   // + conductor, banding, survey, straighten
    ...
    CHECK(t.Has("bandOvalFailAt"));              // banding.json
    CHECK(t.Has("surveyVisualRangeMetres"));     // survey.json
    CHECK(t.Has("straightenSettleHours"));       // straighten.json
