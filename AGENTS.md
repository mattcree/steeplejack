...
A frame of the jack standing in a field proves almost nothing, so the pawn carries console
commands that put him where the test needs him. Pass them with `PLAY_CMDS`, comma separated:

```bash
make play PLAY_CMDS="SJClimb 26,SJCarry,SJDog 26,SJShot 3"
```

| | |
|---|---|
| `SJClimb <m>` | on the ladder at that height, with stack lashed up to it |
| `SJDog <m>` | a dog seated at that height, as if driven cleanly |
| `SJCarry` | a ladder on the shoulder and a full bag of dogs |
| `SJTap <s>` | sound the brickwork after a delay |
| `SJStrain <grip> <nerve>` | drained meters, to see the telegraphs |
| `SJCam` | toggle third and first person |
| `SJShot <s>` | screenshot after a settle |

`SJShot` rather than `shot`: `shot` fires the instant the map opens, catching the camera lag
mid-interpolation, and every automated capture came out smeared. Give it three seconds.

**Build with no editor running.** `make build-game` kills one first, and it has to: with an editor
live, UBT quietly does a *hot-reload* build — it writes `SteeplejackGame-0001.so` and leaves the
base `.so` alone, which is what `-game` loads. The build reports success and `make play` renders
the previous build. The session-start MCP hook keeps an editor up, so this is the normal case, not
the exotic one.
